...
export default class TokenServices {
	...
	private getExpiresAtDate(expiresIn) {
		...
		const milliseconds =
			typeof expiresIn === 'string' ? Helpers.string.toMs(expiresIn) : expiresIn
	...
	private write(token: TokenContract<User>) {
		const payload = {
			...
			expires_at: token.expiresAt
				? token.expiresAt.toFormat(this.dialect.dateTimeFormat)
				: null,
		}
	...
	public async validate(token, name): Promise<UserToken> {
		...
		const provided = await this.read(tokenId, this.createHash(value))
