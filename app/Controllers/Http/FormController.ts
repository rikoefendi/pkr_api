...
export default class FormController {
    ...
    async storeOrUpdate({ request }: HttpContextContract) {
        const formPayload = await request.validate({
            schema: schema.create({
                ...
                questions: schema.array().members(schema.object().members({
                    question: schema.string(),
                    isChallenge: schema.boolean.optional(),
                    isScore: schema.number.optional(),
                    options: schema.array.optional().members(schema.object().members({
    ...
    async show({params}: HttpContextContract){
        const form = await Form.find({_id: params.formId})
        return {
            data: form,
            message: 'success',
            error: false,
